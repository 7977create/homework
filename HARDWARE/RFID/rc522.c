...
extern u8 lcd_clear_flage;								//用于返回主函数时，清屏
...
void RC522_Han(void)
{
    ...
    if(status==MI_OK)// 寻卡成功
    {
				LCD_Clear(WHITE);
				printf("寻卡成功\n");
				LCD_ShowString(10,10,210,24,24,"card searched");
				delay_ms(1000);
        status=MI_ERR;
        status = PcdAnticoll(SN);// 防冲撞
				ShowID(SN);
				LCD_ShowString(10,80,210,24,24,"cardID：");
				delay_ms(1000);
				LCD_ShowString(10,120,210,24,24,SN);
				delay_ms(1000);
//				OpenDoor(SN);
				lcd_clear_flage=1;								//用于返回主函数时，给清屏信号
...
void ShowID(u8 *p)	 //显示卡的卡号，以十六进制显示
{
	...
	for(i =0;i<10;i++)
		...
		if(strcmp(SN,"8790BEB4") == 0)
		{
				printf ( "id正确" );
				printf ( "卡号正确" );
				LCD_ShowString(10,40,210,24,24,"DOOR OPENING:");
				Server_180_Angle();
				delay_ms(1000);
		}
		else
		{
				LCD_ShowString(10,40,210,24,24,"ID ERROR");
...
void OpenDoor(u8 *p)	 //开门
{
	...
	for(i =0;i<10;i++)
		...
		{
				printf ( "卡号正确" );
				LCD_ShowString(10,120,210,24,24,"DOOR OPENING:");
				Server_180_Angle();
				delay_ms(1000);
		}
}
