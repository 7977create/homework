...
void esp8266_start_trans(void)
{
	...
	//让模块连接上自己的路由
	while(esp8266_send_cmd("AT+CWJAP=\"scs-home\",\"scs18285086774\"","WIFI GOT IP",600));
	
	//=0：单路连接模式     =1：多路连接模式
	esp8266_send_cmd("AT+CIPMUX=0","OK",20);
	
	//建立TCP连接  这四项分别代表了 要连接的ID号0~4   连接类型  远程服务器IP地址   远程服务器端口号
	while(esp8266_send_cmd("AT+CIPSTART=\"TCP\",\"192.168.3.66\",1234","CONNECT",200));
...
u8 esp8266_quit_trans(void)
{
	...
	if(result)
		u3_printf("quit_trans failed!");
	else
		u3_printf("quit_trans success!");
	return result;
}
